...
class cptbc_settings_page {
  ...
  public function showcontrols_callback() {
    ...
    print '<select id="showcontrols" name="cptbc_settings[showcontrols]">
      ...
      <option value="custom"'.$cptbc_showcontrols_f.'>'.__('Custom', 'cpt-bootstrap-carousel').'</option>
    </select>';
  }
  ...
  if(count($images) > 0){
    ...
    <div id="cptbc_<?php echo $id; ?>" class="carousel slide" data-ride="carousel" data-interval="<?php echo $atts['interval']; ?>">
      ...
      <?php } else if($atts['showcontrols'] === 'custom' && $atts['twbs'] == '3' &&  $atts['customprev'] != '' &&  $atts['customnext'] != ''){ ?>
        <a class="left carousel-control" href="#cptbc_<?php echo $id; ?>" data-slide="prev"><span class="$atts['customprev'] icon-prev"></span></a>
        <a class="right carousel-control" href="#cptbc_<?php echo $id; ?>" data-slide="next"><span class="$atts['customnext'] icon-next"></span></a>
      <?php } ?>
    </div>
<?php }
